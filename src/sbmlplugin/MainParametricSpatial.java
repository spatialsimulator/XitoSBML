...
public class MainParametricSpatial extends MainSpatial {
	...
	public void run(String arg) {
		/*
		 * Frame[] f =ImageJ.getFrames(); Frame frame = null; for(int i = 0 ; i
		 * < f.length ; i++){ if(f[i].getTitle().equals("ImageJ 3D Viewer"))
		 * frame = f[i]; } if(frame == null){ IJ.error("3D Viewer not opened");
		 * return; } ImageWindow3D win = (ImageWindow3D) frame; univ =
		 * (Image3DUniverse) win.getUniverse();
		 */
		createSBMLDoc();
		gui();
		computeImg();
