...
public class MainSBaseSpatial extends MainSpatial implements PlugIn {
	...
	private void checkLevelAndVersion() {
		if (model.getLevel() == PluginInfo.LOWERSBMLLEVEL) // level 2 change to
															// latest level 2
															// version
			document.setLevelAndVersion(PluginInfo.LOWERSBMLLEVEL,
					PluginInfo.LOWERSBMLVERSION);
		else {
		} // level 3 if new verison comes up check
	...
	private void checkExtension() {
		...
		if (!model.isPackageEnabled("spatial"))
			model.enablePackage(SpatialExtension.getXmlnsL3V1V1(), "spatial",
					true);
