...
import vib.InterpolatedImage.Interpolate;
...
public abstract class MainSpatial implements PlugIn {
	...
	protected void createSBMLDoc() {
		...
		sbmlns.addPackageNamespace("spatial", 1); // add spatial processes
													// package
		// SBML Document
		document = new SBMLDocument(sbmlns); // construct document with
												// namespace
	...
	protected void computeImg() {
		...
		CreateImage creIm = new CreateImage(imgexp.getDomFile(),
				hashSampledValue);
		spImg = new SpatialImage(hashSampledValue, hashDomainTypes,
				creIm.getCompoImg());
	...
	protected void save() {
		SaveDialog sd = new SaveDialog("Save SBML Document", model.getId(),
				".xml");
		...
		try {
			if (name.contains(".xml"))
				libsbml.writeSBMLToFile(document, sd.getDirectory() + "/"
						+ name);
			else
				libsbml.writeSBMLToFile(document, sd.getDirectory() + "/"
						+ name + ".xml");
